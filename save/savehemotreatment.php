...
list($H_id ,$id2) = explode(" | ", $pageid);
 ...
 $q2 = $conn->query ("SELECT * FROM `patientschedule` WHERE BINARY `Hospital_Id` = '$H_id'") or die(mysqli_error());
    $fsched = $q2->fetch_array();   
    $day = $fsched['treatment_day'];
    $time1 = $fsched['treatment_time'];
...
if(ISSET($_POST['treatment_infos'])){
   ...
   $q1 = $conn->query ("SELECT * FROM `treatment` WHERE BINARY `Hospital_Id` = '$H_id' && `treatment_date` = '$date'") or die(mysqli_error());
    ...
    $conn->query ("UPDATE `patientschedule` SET `treatment_status` = '1' WHERE `patientschedule`.`Hospital_Id` = '$H_id'") or die(mysqli_error());
    $conn->query ("INSERT INTO `patientschedule_log` VALUES ('$H_id', '$day', '$time1', '1', '', '$date')") or die(mysqli_error());
